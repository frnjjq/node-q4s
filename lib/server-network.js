  ...
  constructor(store) {
      ...
      socket.on('data', (data) => {
        try {
          ...
          this.emit('handshake-Req', req, (msg) => {
            socket.end(msg.toString(), 'utf8');
          });
        } catch (error) {
          socket.end(ResQ4S.genRes(400).toString(), 'utf8');
        }
      });
    });
    /**
    ...
    this.TCP.on('connection', (socket) => {
      ...
      if (record) {
        ...
        socket.end(ResQ4S.genRes(600).toString(), 'utf8');
      }
    });
